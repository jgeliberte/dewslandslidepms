 ...
 * @link
 * 
 * Additional edits and functionalities 
 * by Kevin Dhale dela Cruz <kevindhaledelacruz@gmail.com>
 */
...
class Migrate extends CI_Controller
{
	...
	/**
	 * Migrates to the '$version' version.
	 *
	 * @param int $version Rollback Migration Version
	 * @return void
	 */
	public function run_version($version = null)
	{
		if (!is_null($version)) {
			...
			show_error("Version number not specified.");
		}
	}
	...
	public function info ()
	{
		...
		echo PHP_EOL . "Current migration version (based on /config/migration 'migration_version'): $migration_version" . PHP_EOL;
		...
		if ((is_array($migrations)) && (count($migrations) > 0))
		{
			...
			foreach ($migrations as $key => $value) 
			{
				...
				{	
					$string = '| T   | ';
					if (intval($this->_get_migration_number($value)) > $version)
					{
						$string = '| F   | ';
					}
	...
	protected function _get_file_content($file_name = '')
	{
		...
		else
		{
			...
			$file_content .= '			"id" => array(' . PHP_EOL;
			$file_content .= '				"type" => "INT",' . PHP_EOL;
			$file_content .= '				"null" => FALSE,' . PHP_EOL;
			$file_content .= '				"auto_increment" => TRUE,' . PHP_EOL;
			$file_content .= '			),' . PHP_EOL;
			$file_content .= '			"reference_id" => array(' . PHP_EOL;
			$file_content .= '				"type" => "INT"' . PHP_EOL;
			$file_content .= '			),' . PHP_EOL;
			$file_content .= '			"name" => array(' . PHP_EOL;
			$file_content .= '				"type" => "VARCHAR",' . PHP_EOL;
			$file_content .= '				"constraint" => "100",' . PHP_EOL;
			$file_content .= '				"unique" => TRUE' . PHP_EOL;
			$file_content .= '			),' . PHP_EOL;
			$file_content .= '			"created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP"' . PHP_EOL;
			$file_content .= '		);' . PHP_EOL;
			$file_content .= '		// Add primary key'  .  PHP_EOL;
			$file_content .= '		$this->dbforge->add_key("id", TRUE);'  .  PHP_EOL;
			$file_content .= '		$this->dbforge->add_field($fields);'  .  PHP_EOL;
